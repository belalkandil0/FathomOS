...
public class LicenseManager : IDisposable
{
    ...
    public async Task<LicenseValidationResult> ForceServerCheckAsync()
    {
        ...
        try
        {
            ...
            System.Diagnostics.Debug.WriteLine($"DEBUG: ForceServerCheckAsync - server response:");
            System.Diagnostics.Debug.WriteLine($"DEBUG:   IsValid: {onlineResult.IsValid}");
            System.Diagnostics.Debug.WriteLine($"DEBUG:   Status: {onlineResult.Status}");
            System.Diagnostics.Debug.WriteLine($"DEBUG:   Message: {onlineResult.Message}");
            ...
            // FIXED: Only update cache if we got a REAL server response
            // Do NOT update cache if message indicates server was unreachable
            bool isServerError = onlineResult.Message.Contains("Unable to reach") || 
                                 onlineResult.Message.Contains("timeout") ||
                                 onlineResult.Message.Contains("Using offline");
            
            if (!isServerError)
            {
                // Got a real response from server - update cache
                System.Diagnostics.Debug.WriteLine($"DEBUG: Updating cache with server response");
                _cachedResult = onlineResult;
                _lastValidation = DateTime.UtcNow;
            ...
            {
                // Server error - keep existing cache, return offline validation
                System.Diagnostics.Debug.WriteLine($"DEBUG: Server error - returning offline validation, NOT updating cache");
                return _cachedResult ?? currentLicense;
            }
        ...
        catch (HttpRequestException)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"DEBUG: HttpRequestException - returning cached/offline result");
        ...
        catch (TaskCanceledException)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"DEBUG: TaskCanceledException - returning cached/offline result");
            return _cachedResult ?? _validator.ValidateCurrentLicense();
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Gets the cached license validation result without triggering a new check.
    ...
    /// <summary>
    /// Alias for IsModuleLicensed - checks if a module is accessible under the current license.
    /// Use this method name for consistency with documentation.
    /// </summary>
    /// <param name="moduleId">The module ID (e.g., "SurveyListing", "TideAnalysis")</param>
    /// <returns>True if the module is licensed and accessible</returns>
    public bool HasModuleAccess(string moduleId) => IsModuleLicensed(moduleId);
...
public class LicenseStatusInfo
{
    ...
    /// <summary>
    /// Alias for IsModuleLicensed - checks if a module is accessible.
    /// </summary>
    public bool HasModuleAccess(string moduleId) => IsModuleLicensed(moduleId);
